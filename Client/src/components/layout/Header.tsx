        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginLeft: minimal ? 'auto' : '0' }}>
          ...
          {token ? (
            <div className="nav-item" ref={profileRef} style={{ position: 'relative' }}>
              ...
              >
                <button onClick={() => { setProfileOpen(false); navigate('/profile'); }}><User size={16} /> Profile</button>
                <button onClick={() => { setProfileOpen(false); navigate('/settings'); }}><Settings size={16} /> Settings</button>
                {userRole === 'DOCTOR' && (
